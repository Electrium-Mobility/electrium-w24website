    ...
    return (
      ...
      <ul className="mx-auto lg:w-1/2 md:w-full" style={{ listStyleType: 'none', padding: 0 }}> 
        {faqs.map((faq, index) => (
          ...
          <li key={index} className="rounded-lg my-8">
            {/* FAQ question */}
            <div onClick={() => handleFaqSelection(index)} style={{ cursor: 'pointer' }}>
                <h2>
                    <button className="font-nunito flex text-base justify-between items-center p-5 w-full" aria-expanded="false" aria-controls="accordion-collapse-body-6">
