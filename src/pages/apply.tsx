...
const ApplicationForm = () => {
    ...
    const CheckboxField = ({ name, label, caption = <></>, options = []}) => (
        <div className="grid grid-cols-1 mb-5">
            ...
            <label htmlFor={name} className="text-gray-500 text-sm">{caption}</label>
            <Field type="checkbox" name={name} className="form-select mt-2 text-charcoal-600 border border-charcoal-300 rounded-md px-4 py-3 focus:outline-none focus:ring-green-700 focus:border-green-700">
                <option value="">{"-Select option-"}</option>
                {options.map(option => (
                    <option key={option} value={option}>{option}</option>
                ))}
                                        ...
                                        {() => (
                                            <Form>
                                                ...
                                                <TextField name="interests"
                                                    ...
                                                    caption={
                                                        <>
                                                            This is for us to get to know you, and does not have an impact on your application :)
                                                        </>
                                                    }
                                                    type="text"
                                                />
                                                <RadioField name="heardSource" label="How did you hear about Electrium Mobility?" options={SELECT_HEARD_SOURCE} />
                                                <RadioField name="role" 
                                                    label="What role are you interested in?"
                                                    caption={
                                                        <>
                                                            You can learn more about what the various roles do <Link to="/contact" className="text-green-600 text-bold">here</Link>.
                                                        </>
                                                    }
